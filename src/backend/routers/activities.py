...
def get_available_difficulty_levels() -> List[str]:
    """Get a list of all difficulty levels that are used in activities"""
    # Aggregate to get unique difficulty levels across all activities
    pipeline = [
        {"$match": {"difficulty": {"$exists": True}}},  # Only activities with difficulty field
        {"$group": {"_id": "$difficulty"}},
        {"$sort": {"_id": 1}}  # Sort difficulty levels alphabetically
    ]
    
    difficulty_levels = []
    for level_doc in activities_collection.aggregate(pipeline):
        difficulty_levels.append(level_doc["_id"])
